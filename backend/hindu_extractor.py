...
class HinduExtractor:
    def __init__(self):
        ...
        self.delhi_keywords = ['delhi', 'new delhi', 'ncr', 'noida', 'gurgaon', 'gurugram', 'faridabad', 'dwarka', 'rohini']
        self.rss_feeds = [
    ...
    def _build_article(self, url: str, fallback_title: str = '') -> Dict:
        result = self.text_extractor.extract(url, source='The Hindu')
        return {
            'url': result['url'],
    ...
    def extract(self, max_articles: int = 200, seen_urls: set = None) -> List[Dict]:
        ...
        print(f"\n{'='*60}")
        print(f"✓ The Hindu Total  : {len(all_articles)} articles")
